...
import { ShieldCheck, Mail, Phone, Calendar, ArrowLeft, KeyRound, LogOut, Edit, CheckCircle2 } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6 pb-16">
      ...
      <div className="flex items-center justify-between border-b border-[#8B0000]/10 pb-4">
        <div className="flex items-center gap-3">
          <Link href="/admin/dashboard" className="p-2 rounded-xl bg-white border border-[#8B0000]/15 text-[#8B0000] hover:bg-[#8B0000] hover:text-white transition-all">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div>
            <h1 className="text-2xl font-extrabold text-[#1a1008]">Admin Account Profile</h1>
            <p className="text-xs text-[#6b5840]">System administrator identity, privileges, and access credentials</p>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card rounded-3xl bg-white border border-[#8B0000]/15 shadow-sm p-6 space-y-6">
        <div className="flex items-center gap-5 border-b border-[#8B0000]/10 pb-6">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-extrabold text-[#1a1008]">{user?.name || 'Administrator'}</h2>
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-red-100 text-red-900 border border-red-300">
                Super Admin
              </span>
            </div>
            <p className="text-xs text-[#6b5840]">{user?.email || 'admin@royalrestaurant.com'}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div className="bg-[#F8F5F0] p-4 rounded-2xl space-y-1 border border-[#8B0000]/10">
            ...
            <span className="font-extrabold text-[#1a1008]">{user?.name || 'Administrator'}</span>
          </div>

          <div className="bg-[#F8F5F0] p-4 rounded-2xl space-y-1 border border-[#8B0000]/10">
            <span className="text-[10px] font-bold uppercase text-[#a09070] block">Email Address</span>
            <span className="font-extrabold text-[#1a1008]">{user?.email || 'admin@royalrestaurant.com'}</span>
          </div>

          <div className="bg-[#F8F5F0] p-4 rounded-2xl space-y-1 border border-[#8B0000]/10">
            <span className="text-[10px] font-bold uppercase text-[#a09070] block">Phone Number</span>
            <span className="font-extrabold text-[#1a1008] font-mono">{user?.phone || '+91 98765 00000'}</span>
          </div>

          <div className="bg-[#F8F5F0] p-4 rounded-2xl space-y-1 border border-[#8B0000]/10">
            <span className="text-[10px] font-bold uppercase text-[#a09070] block">System Role</span>
            <span className="font-extrabold text-[#8B0000]">ADMIN</span>
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-[#8B0000]/10 flex flex-wrap items-center justify-between gap-3 text-xs font-bold">
          <div className="flex items-center gap-2">
            <button
              onClick={() => handleAction('Edit Admin Profile initialized.')}
            ...
            <button
              onClick={() => handleAction('Admin password change token issued.')}
