...
export default function BottomBar() {
  ...
  return (
    /* Only visible on mobile/tablet (below md) */
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-[#8B0000]/10 shadow-[0_-4px_20px_rgba(139,0,0,0.08)]">
        ...
        {TABS.map(({ label, href, icon: Icon }) => {
          ...
          return (
            ...
            >
              {/* Active top bar */}
              {isActive && (
                <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-[#8B0000] rounded-full" />
              )}
